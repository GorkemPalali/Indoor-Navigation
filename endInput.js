...
//Sort names2 in ascending order
...
endInput.addEventListener("keyup", (e) => {
  ...
  for (let i of sortedNames2) {
    //convert endInput to lowercase and compare with each string
    if (
      i.toLowerCase().startsWith(endInput.value.toLowerCase()) &&
      endInput.value != ""
    ) {
      //create li element
      ...
      let word = "<b>" + i.substr(0, endInput.value.length) + "</b>";
      word += i.substr(endInput.value.length);
